...
# ---------------------------------------------------------
# Archivo : 06_render_reporte.R
# Propósito: renderizar reporte HTML con Quarto
# =========================================================

cat("Renderizando reporte del cap\u00edtulo 6 con Quarto...\n")

archivo_reporte   <- file.path(root_dir, "reporte_cap6.qmd")
dir_salida        <- file.path(root_dir, "outputs", "reportes")
archivo_salida    <- "reporte_cap6.html"
salida_html_final <- file.path(dir_salida, archivo_salida)
salida_html_tmp   <- file.path(root_dir, archivo_salida)
carpeta_files_tmp   <- file.path(root_dir, "reporte_cap6_files")
carpeta_files_final <- file.path(dir_salida, "reporte_cap6_files")
...
salida <- system2(
  command = quarto_bin,
  args    = c("render", "reporte_cap6.qmd", "--to", "html", "--output", archivo_salida),
