...
# --- Tablas ---
guardar_xlsx(as.data.frame(head(biol)[, 1:6]),
...
resultado_casoA1 <- list(
  biol_head        = as.data.frame(head(biol)[, 1:6]),
...
# d.beta resultados generales
tab_dbeta <- data.frame(
  SSTotal = d.beta$beta["SSTotal"],
  BDTotal = d.beta$beta["BDTotal"]
)
...
guardar_xlsx(round(as.data.frame(t(d.beta$beta)), 3),
