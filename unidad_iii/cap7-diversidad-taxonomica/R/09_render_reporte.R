...
cat("Renderizando reporte del cap\u00edtulo 7 con Quarto...\n")

archivo_reporte   <- file.path(root_dir, "reporte_cap7.qmd")
dir_salida        <- file.path(root_dir, "outputs", "reportes")
archivo_salida    <- "reporte_cap7.html"
salida_html_final <- file.path(dir_salida, archivo_salida)
salida_html_tmp   <- file.path(root_dir, archivo_salida)
carpeta_files_tmp   <- file.path(root_dir, "reporte_cap7_files")
carpeta_files_final <- file.path(dir_salida, "reporte_cap7_files")
...
salida <- system2(
  command = quarto_bin,
  args    = c("render", "reporte_cap7.qmd", "--to", "html",
...
if (dir.exists(carpeta_files_tmp)) {
  ...
  if (!ok_dir) {
    ...
    file.copy(archivos_aux, carpeta_files_final,
