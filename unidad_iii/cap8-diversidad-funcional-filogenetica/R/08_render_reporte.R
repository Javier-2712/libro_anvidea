...
# ---------------------------------------------------------
# Archivo : 08_render_reporte.R
# Propósito: renderizar reporte HTML con Quarto
# =========================================================

cat("Renderizando reporte del capítulo 8 con Quarto...\n")

if (!exists("root_dir")) root_dir <- getwd()

archivo_reporte   <- file.path(root_dir, "reporte_cap8.qmd")
dir_salida        <- file.path(root_dir, "outputs", "reportes")
archivo_salida    <- "reporte_cap8.html"
salida_html_final <- file.path(dir_salida, archivo_salida)
salida_html_tmp   <- file.path(root_dir, archivo_salida)
carpeta_files_tmp   <- file.path(root_dir, "reporte_cap8_files")
carpeta_files_final <- file.path(dir_salida, "reporte_cap8_files")
...
salida <- system2(
  command = quarto_bin,
  args    = c("render", "reporte_cap8.qmd", "--to", "html", "--output", archivo_salida),
...
if (dir.exists(carpeta_files_tmp)) {
  ...
  if (!ok_dir) {
    ...
    archivos_aux <- list.files(carpeta_files_tmp, full.names = TRUE, all.files = TRUE, no.. = TRUE)
